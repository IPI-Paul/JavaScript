  ...
  const main = async () => {
    const plot = 
      scatterPlot()
        ...
        .margin({top: 20, right: 20, bottom: 30, left: 70, x_axis: 10, y_axis: 20})
        .radius(5)
        .interval(wait)
        .xAxisTitle(columns[0].toProperCase())
        .yAxisTitle(columns[3].toProperCase());
    ...
    const interval = setInterval(() => {
      ...
      plot
        .xValue((d) => d[columns[i]])
        .xAxisTitle(columns[i].toProperCase());
      svg.call(plot);

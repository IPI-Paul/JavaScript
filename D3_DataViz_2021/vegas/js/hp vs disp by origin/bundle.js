(function (vega, vegaLite, vl, vegaTooltip, d3) {
  ...
  const viz = vl__default["default"].markCircle({
    size: 300,
    opacity: 0.5
  })
    .encode(
      vl__default["default"].x().title('Displacement').fieldQ('displacement').scale({ zero: false }),
      vl__default["default"].y().title('Horsepower').fieldQ('horsepower').scale({ zero: false }),
      vl__default["default"].color().title('Origin').fieldN('origin'),

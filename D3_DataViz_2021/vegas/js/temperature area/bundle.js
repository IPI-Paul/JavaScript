(function (vega, vegaLite, vl, vegaTooltip, d3) {
  ...
  const viz = vl__default["default"].markArea({
    size: 3,
    opacity: 1
  })
    .encode(
      vl__default["default"].x().title('Timestamp').fieldT('timestamp').scale({ zero: false }),
      vl__default["default"].y().title('Temperature').fieldQ('temperature').scale({ zero: false }),
      vl__default["default"].tooltip().fieldN('temperature').format('.2f')
    );
  /*
    fieldN = Nominal
    fieldT = Time
